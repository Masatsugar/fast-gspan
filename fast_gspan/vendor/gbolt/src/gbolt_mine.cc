// Modified: 2025-12-07 - Added max_num_vertices support
// Modified: 2025-12-08 - Added projection size limit to prevent memory explosion
#include <gbolt.h>
#include <graph.h>
#include <path.h>
#include <common.h>
...
// Maximum projection size to prevent memory explosion on dense graphs
#ifndef MAX_PROJECTION_SIZE
#define MAX_PROJECTION_SIZE 500000
#endif
...
void GBolt::report(const DfsCodes &dfs_codes, const Projection &projection,
  ...
  // NOTE:
  // We output full DFS-code tuple so that Python wrapper can reconstruct
  // the same canonical DFS code representation as python gSpan.
  // Format: e <from> <to> <from_label> <edge_label> <to_label>
  for (auto i = 0; i < dfs_codes.size(); ++i) {
    ss << "e " << dfs_codes[i]->from << " " << dfs_codes[i]->to
      << " " << dfs_codes[i]->from_label
      << " " << dfs_codes[i]->edge_label
      << " " << dfs_codes[i]->to_label << std::endl;
  }
  ss << "x: ";
  int prev = 0;
...
void GBolt::mine_subgraph(
  ...
  // Modified: 2025-12-07 - Check max_num_vertices before further mining
  // Count vertices in current pattern (max vertex id + 1)
  int current_num_vertices = 0;
  for (auto i = 0; i < dfs_codes.size(); ++i) {
    current_num_vertices = std::max(current_num_vertices, dfs_codes[i]->from + 1);
    current_num_vertices = std::max(current_num_vertices, dfs_codes[i]->to + 1);
  }
  
  // If max_num_vertices_ is set (> 0) and current pattern exceeds it, skip
  if (max_num_vertices_ > 0 && current_num_vertices > max_num_vertices_) {
    return;
  }
  ...
  for (auto it = projection_map_backward.begin(); it != projection_map_backward.end(); ++it) {
    ...
    // Modified: 2025-12-08 - Skip if projection is too large to prevent memory explosion
    if (projection.size() > MAX_PROJECTION_SIZE) {
      continue;
    }
